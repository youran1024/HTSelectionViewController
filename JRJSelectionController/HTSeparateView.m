...
- (void)initilization
{
    ...
    self.backgroundColor = [UIColor guideViewBackColor];
...
- (UIImageView *)leftIndicateView
{
    if (!_leftIndicateView) {
        ...
        _leftIndicateView.backgroundColor = [UIColor stockTitleViewBackGray];
...
- (UIImageView *)rightIndicateView
{
    if (!_rightIndicateView) {
        ...
        _rightIndicateView.backgroundColor = [UIColor stockTitleViewBackGray];
...
- (void)selectButtonAtIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    [_selectButton setTitleColor:HTHexColor(0x595959) forState:UIControlStateNormal];
    _selectButton = selectButton;
    [_selectButton setTitleColor:HTHexColor(0xde3031) forState:UIControlStateNormal];
    ...
    [UIView animateWithDuration:animated ? .25 : 0 animations:^{
        if (_slideType == SlideViewTypeLine) {
            rect.origin.y = CGRectGetHeight(self.frame) - SlideLineHeight;
...
-(void)initilizeView
{
    ...
    for (NSString *title in _titles) {
        ...
        width += CGRectGetWidth(button.frame) + _globalMargin;
    }
...
- (void)adjustContentOffset:(UIButton *)button
{
    ...
    CGFloat offsetX = self.contentSize.width - screen.bounds.size.width;
    CGPoint offset;
    if (button.left > 170) {
...
- (void)layoutSubviewsNow
{
    ...
    CGFloat margin = (CGRectGetWidth(self.frame) - 2 * _globalMargin - buttonWidth) / (_buttons.count - 1) - .3;
    ...
    for (UIButton *button in _buttons) {
        ...
        if (index == 0) {
            rect.origin.x = _globalMargin / 2.0f;
            rect.size.width += _globalMargin / 2.0f + margin / 2.0f;
    ...
    self.contentSize = CGSizeMake(minWidth, CGRectGetHeight(rect));
